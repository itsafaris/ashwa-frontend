...
async function fetchOptions(
  ...
  SelectOptionType[] | { error: { message: string; [prop: string]: any } }
> {
  try {
...
export function Location({ placeholder }: LocationProps) {
  ...
  const loadOptionsDebounced = useCallback(
    debounce(
      (inputValue: string, cb: (options: SelectOptionType[]) => void) => {
        if (!quizConfig.locationApiKey) {
          console.warn("Location api key is missing, won't fetch locations");
          cb([]);
          return;
        }
